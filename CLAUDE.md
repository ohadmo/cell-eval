...
- **DE metrics** (`_de.py`, `_gsea_nes.py`, `_aucell_auc.py`, `_progeny.py`, `_dorothea.py`, `_collectri.py`): overlap/precision at N, spearman correlations, direction match, significant gene recall, ROC/PR AUC, and VCC-only pathway/regulator metrics for GSEA NES Spearman, AUCell AUC Spearman, PROGENy activity Spearman, DoRothEA TF activity Spearman, and CollecTRI TF activity Spearman
